...
ensure_python_and_venv() {
  ...
  # Determine Python 3 package
  PYTHON3_PKG="python3"
  PYTHON_VENV_PKG="python3-venv"

  # Install Python 3 and venv if missing
  if ! command -v python3 >/dev/null 2>&1; then
    info "Installing Python 3 and pip..."
    apt_install "$PYTHON3_PKG" "$PYTHON_VENV_PKG" python3-pip
  else
    ok "Python3 already installed"
    # Ensure venv package exists
    if ! python3 -m venv --help >/dev/null 2>&1; then
      info "Installing python3-venv for current Python..."
      apt_install "$PYTHON_VENV_PKG"
    fi
  fi
