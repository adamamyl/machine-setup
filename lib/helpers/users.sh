...
add_user_to_group() {
  ...
  getent group "$group" >/dev/null || _root_cmd "groupadd -f $group"

  # Only add if user is not already in the group
  if ! id -nG "$user" | grep -qw "$group"; then
    _root_cmd "usermod -aG $group $user"
  else
...
users_to_groups_if_needed() {
  ...
  shift
  local groups=("$@")
  _root_cmd "id $user" >/dev/null 2>&1 || _root_cmd "useradd $user -m"
  for group in "${groups[@]}"; do
    _root_cmd "getent group $group || groupadd -f $group"
    _root_cmd "usermod -aG $group $user"
  done
}
...
create_if_needed_ssh_dir() {
  ...
  _root_cmd "mkdir -p -m 700 $ssh_dir"
  _root_cmd "chown $user:$user $ssh_dir"
  echo "$ssh_dir"
}
...
set_homedir_perms_recursively() {
  local user="$1"
  local dir="$2"
  _root_cmd "chown -R $user:$user $dir"
  _root_cmd "find $dir -type f -exec chmod 644 {} \;"
  _root_cmd "find $dir -type d -exec chmod 755 {} \;"
}

set_ssh_perms() {
  local user="$1"
  local ssh_dir="$2"
  _root_cmd "chmod 700 $ssh_dir"
  _root_cmd "chown $user:$user $ssh_dir"
  _root_cmd "find $ssh_dir -type f -exec chmod 600 {} \;"
}
